...
- 2026-09-23 — **Phase 12, banking & reconciliation (12A–12E).** The audit came FIRST and found a defect in SHIPPED code: a statement line Phase D had matched to a receipt stayed `pending_review`, and accepting it posted the same money twice — fixed before anything new was built. `ERPNEXT`/`ODOO`, read from `version-15`/`17.0` source for matching and transfers (cited in the pack §2.3): ERPNext allocates a Bank Transaction across vouchers and records an own-account transfer as ONE Payment Entry posting both legs — the shape built; Odoo pairs two payments through a transfer account. Both DELETE a reconciliation link on undo; this product supersedes it. 🔴 **Not read from source, and marked `UNVERIFIED` in the pack rather than asserted:** how either treats a closing difference, and whether either locks a bank per reconciliation — the no-plug rule and the lock rest on the brief, not on them. No Saudi or VAT consequence is expected from reconciliation or from a transfer between own accounts (no other party, no supply) — `REASONED`, no primary text read, no VAT line written either way. No new accountant question; **Z-AP1 remains pending** and nothing here encodes it. Record: [`docs/product/phase-12-banking-reconciliation-decision-pack.md`](product/phase-12-banking-reconciliation-decision-pack.md).
- 2026-09-24 — **Z-AP1 answered (A), and the bank-reconciliation lock classified.** Z-AP1 — `ACCOUNTANT DECISION`: claim input VAT in the supplier advance tax invoice's applicable VAT period, with controls preventing duplicate claim when the final invoice is issued. `AUTHORITATIVE (Saudi/GCC)` read: GCC Agreement Art. 23(1), 44(2), 48(1)(a); IR Art. 49(7), 49(8), 53(1)(a)(2), 40(6) (repo English text; Arabic not held locally); XML Implementation Standard §9.5 and its worked example — the final invoice's VAT is GROSS and only KSA-32 names what was already claimed, so netting at the final invoice is effectively required. The texts QUALIFY the decision in three ways, all built: timing is a permission (IR 49(8)), so the product takes the earliest period; a refund is the supplier's credit note in its own period (IR 40(6)); and the netted VAT is the supplier's own KSA-32. 🔴 No official ZATCA text on the BUYER side of an advance or its return box was readable (the Input Tax Deduction guideline URL returns 404): `UNVERIFIED`. `STANDARD (IFRS)`: IAS 32 AG11, IAS 2.11, IAS 16.16(a). `ODOO` 18.0 nets the down payment on the final bill with the same taxes; `ERPNEXT` has no VAT-on-advance logic (its `advance_tax` is TDS). Bank lock — classification **D**: the Commercial Books Law (Arabic, BOE), VAT IR Art. 66 (English), SOCPA (search) and IFRS say nothing specific; ERPNext and Odoo lock per company, never per reconciliation. 🔴 **Completed bank reconciliation locking is an internal Saudi Ledger control/product decision, not an identified Saudi statutory/accounting requirement.** Record: [`docs/product/phase-12-banking-reconciliation-decision-pack.md`](product/phase-12-banking-reconciliation-decision-pack.md) §8–§9.
