...
ladder, which is where tenancy and authorization are established (§4). Business
logic never talks to the database directly — it goes through repositories, with
one sanctioned exception (§3). Background work runs in-process, not on a queue
service.
...
### 3.3 API contract: OpenAPI-first with codegen
...
### 4.2 The identity layer sits outside RLS — deliberately
...
### 4.4 The platform-operator boundary
...
- *A guard that exempts a class from the thing designed to exclude it* — a route
  ...
  **positional** facts, and they are now mechanically checked (§4.6).

### 4.6 The privilege surface map
...
### 4.7 Other properties worth knowing
