...
| GOVT_FEES, GOVT_GRANTS, GOSI_EXPENSE/PAYABLE | O | The sovereign-capacity and no-supply/no-consideration rules live in the **GCC VAT Framework Agreement** (Art. 9 area), which was NOT read. Per the owner's rule these are not resolved by reasoning. | 🔴 **Queue C11**: read the GCC VAT Agreement — likely desk research exactly like this pass, advisor only if the text is ambiguous. |
