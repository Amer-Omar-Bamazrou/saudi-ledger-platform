...
# 17. The accounting-period correction, the four liabilities, and bad debts: as built (2026-09-22)

**Status (2026-09-22): BUILT on `feat/ap-accounting-period-correction` (on top of `main` at the AP-4 merge `77fcf3f4`), on the accountant's answers of 2026-09-22 — recorded here verbatim in substance and applied without addition. Current state authority: [CLAUDE.md §2](../../CLAUDE.md).**

## 17.1 The accountant's answers, and where each landed

| # | Answer (2026-09-22) | Built as | Class |
| --- | --- | --- | --- |
| **1(a)** | Tax point = the payment receipt date. Receipt 30 Jan, 386 issued 3 Feb ⇒ VAT in JANUARY; the 386's supply date = 30 Jan; its IssueDate = 3 Feb. Three concepts, kept apart. | The 386's accounting/VAT date (`invoices.date`) is the receipt date, ALWAYS — a caller cannot choose another (`advance_invoice_date_is_tax_point`). Its supply date (KSA-5, `cac:Delivery/cbc:ActualDeliveryDate`) is the receipt date — a new `supplyDate` field on the e-invoice input, null (= issue date) for every other document. Its IssueDate/IssueTime are `issued_at`, the real issuance, never overwritten. The E2 entry is dated the receipt date; the return files the 386 in the receipt's period. | ACCOUNTANT DECISION — RECORDED; GCC Agreement Art. 23(1), IR Art. 53(1)(a)(2) |
| **1(b)** | An unidentified January receipt later established as a taxable advance: VAT belongs to January; correction / amended-return treatment. | A LOCKED receipt month FAILS CLOSED: `423 advance_tax_point_period_locked` at create and at approval, naming the remedy — reopen the month, issue the 386 at its tax point, close it again, and correct the return under **IR Art. 63**: an understated net tax is corrected by a submission within 20 days of discovery (63(1)), or, below SAR 5,000, in the next return (63(3)); an overstatement in any later return within five years (63(2), (4)). Nothing is posted into the locked month, nothing slides to the current month, no plug. **No conflict** between the answer and the text: Art. 63 is the *filing* mechanism for exactly the position the accountant states (the VAT belongs to the receipt's period; the filed return for that period was short). | ACCOUNTANT DECISION — RECORDED; IR Art. 63 read 2026-09-22 (ZATCA English text, Arabic prevails) |
| **2** | Refundable security deposit: its own liability, not unidentified/suspense; outside VAT while genuinely refundable and unavailable for use. Four liabilities: unidentified/erroneous receipts · customer advances · refundable security deposits · credit-note/refund balances. | Three deposit-side accounts (+ `CUSTOMER_CREDITS`, unchanged): `UNIDENTIFIED_RECEIPTS`, `CUSTOMER_DEPOSITS`, `SECURITY_DEPOSITS_HELD`, seeded system accounts; the classification → account map is ONE seam (`customerCreditPolicy.ts` `depositLiabilityAccount()`): unknown/erroneous → unidentified, advance → deposits, security_deposit → security deposits, **no record → CUSTOMER_DEPOSITS (a stated presumption: money an identified customer sent on account is presumed a deposit until the business says otherwise; the review list makes the silence visible)**. The receipt's own entry credits the classification's account; a classification that changes the account posts ONE reclassification entry (`RECLASS-…`, dated `effectiveDate`, an open month, never before the receipt) for the on-account balance; allocation, unallocation and refund read the receipt's CURRENT account; a security deposit cannot be allocated (`security_deposit_not_allocatable` — reclassify first) and cannot receive a 386 (AP-1's rule); a deposit with an issued, uncancelled 386 cannot leave CUSTOMER_DEPOSITS (`advance_invoiced_cannot_reclassify`). The deposits invariant reconciles the three accounts together. | ACCOUNTANT DECISION — RECORDED; the presumption for an unstated receipt is a PRODUCT DECISION, named as one |
| **3** | A credit note required today on a pre-e-invoicing invoice is issued electronically through Fatoora; no manual route. | Recorded; built in the migration follow-ups (§18) — the historical document's identity gates it. | ACCOUNTANT DECISION — RECORDED |
| **4** | Art. 40(9): a recovery after bad-debt relief requires a NEW tax invoice; VAT accounted; a new ZATCA document, not a posting rule. | `document_type = recovery_invoice` (ZATCA 388), its own type so no receivable reader admits it — §17.3. | ACCOUNTANT DECISION — RECORDED; IR Art. 40(7)–(9) read 2026-09-22 |
| **5** | Migration correction 100,000 → 90,000: the other side is Opening Retained Earnings; no OBE plug. | Built in the migration follow-ups (§18). | ACCOUNTANT DECISION — RECORDED (A5's family) |

## 17.2 The bad-debt relief (Art. 40(7)) — a structured, posted fact

`POST /invoices/{id}/bad-debt-relief` (`badDebtService.writeOffWithRelief`; approver-level): ONE act, because the relief presupposes the write-off (40(7)(d)). Refused by name unless every condition holds — an issued tax invoice / debit note with a customer; not a migrated opening item (`opening_item_relief_is_migration_fact` — the previous system's fact); not already written off; **twelve months since the supply** (`bad_debt_relief_too_early`, the earliest date named); a certificate reference; legal-procedure evidence above SAR 100,000 unpaid (`bad_debt_legal_procedures_required`). Posts `WO-<number>`, dated `claimedOn` in an open month: `Dr Bad debts (net share) · Dr VAT Payable (the relief) / Cr AR (the unpaid consideration, customer)`. The row gains `written_off_amount` and the relief facts (`bad_debt_relief_claimed_on / _vat_amount / _return_period / _certificate_ref / _legal_ref / _source ('recorded' | 'migrated') / _journal_entry_id` — a CHECK makes them all-or-nothing); the invoice stays ISSUED. **Every outstanding reader subtracts `written_off_amount`** (the customer ledger, AR ageing, the statement — a `bad_debt_write_off` event — the position, allocation and note ceilings, the migration reconciliation, the invariants). The VAT return shows the relief in **box 7** for the claim period (box 8 = box 6 + box 7), with the components listed; a migrated relief was in the previous system's return and never appears in ours.

## 17.3 The Art. 40(9) recovery document — `recovery_invoice`

`POST /invoices/{id}/bad-debt-recoveries` (`badDebtService.createRecovery`; create-level; a DRAFT, approved like any invoice): the receivable must carry relief (`recovery_requires_relief`), the receipt must be the same customer's and dated on/after the relief (`recovery_before_relief`); the amount is bounded — for a receivable written off HERE, by the receipt's unapplied balance and by what was written off and not yet declared; for a MIGRATED relief, by the receipt's own allocation to the open item not yet declared (`recovery_requires_allocation`, `recovery_exceeds_allocation`). The VAT rate: the receivable's own for a native invoice; the migration's `historicalVat` or an explicit `vatRate` for a migrated item — **never a default** (`recovery_rate_unknown`: the rate was 5 % before 1 July 2020 and 15 % after). Dated at the **tax point = the receipt date** (Art. 40(9): "the Tax Period in which the payment occurs"); supply date (KSA-5) the same; IssueDate the real issuance; a locked receipt month is refused with the Art. 63 explanation. The XML: a **388** with `cac:BillingReference` naming the original tax invoice (BG-3 — the standard restricts nothing; BR-KSA-56 only *mandates* it for 381/383), a note stating the article, the original, the relief date and that the amount has been received; `PayableAmount` = total (BT-113 is tied to 386 adjustment lines by BR-KSA-73 — *reasoned-not-verified* as the right way to say "already paid"). At approval, under the locks: `Dr Bad debts / Cr VAT Payable` for the VAT (payable again, in the payment's period) and, for a receivable written off here, `Dr <the receipt's liability> / Cr Bad debts` for the gross with an allocation receipt → document (the receipt's unapplied balance, the document's `paid_amount` and the GL agree — `status paid`); a migrated item is settled by the receipt's own D-4 allocation and the document carries the VAT leg only (`status sent`, never a receivable). Refused: the front door (`recovery_invoice_via_recovery`), draft line edits (`advance_invoice_line_derived`), payment (`recovery_invoice_not_payable`). Readers: `NON_RECEIVABLE_DOCUMENT_TYPES` gains the type; `isVatOnlyDocumentType` covers the 386, the 381-against-386 and the recovery invoice together.

## 17.4 What ZATCA's live sandbox attested (2026-09-22, `POST /compliance/invoices`)

| Document | Result |
| --- | --- |
| 386 with KSA-5 = 2026-01-30 and IssueDate 2026-04-01 (fixture) · 386 from REAL ROWS (receipt 30 Jan, issued today) | `PASS`, no warnings, **CLEARED** |
| Art. 40(9) recovery 388 with a BillingReference to the original and KSA-5 = 2026-03-10 < IssueDate (fixture) · the same from REAL ROWS after a write-off, a relief and a receipt | `PASS`, no warnings, **CLEARED** |
| planted negative — a 388 whose BillingReference carries an EMPTY id | flagged (the reference field is READ, so the accepted reference is evidence) |
| recorded, not asserted — a 386 whose KSA-5 is LATER than its IssueDate | `PASS`, CLEARED, no message: **the sandbox attests nothing about the supply date's relation to the issue date** |

`ap-period-correction-zatca-live.test.ts` (6, live). The shipped SDK is unchanged in what it can say (divergences log §15).

## 17.5 Tests and the walk

- `ap-period-correction.test.ts` (7, real rows): 1(a) — January receipt, 386 issued today: VAT in January (box 1/6), E2 dated 30 Jan, KSA-5 = 30 Jan, IssueDate today, an ordinary invoice's KSA-5 unchanged; 1(b) — the locked month refuses with Art. 63 named (20 days, SAR 5,000, reopen), nothing posted, nothing slides; reopened, the 386 issues at its tax point; 2 — the four liabilities: stated at receipt, the presumption, a security deposit not allocatable / not 386-able / refunded from its account / reclassified by ONE dated entry, an unknown receipt allocated out of unidentified, an erroneous one refunded, a credit balance untouched, an invoiced advance not reclassifiable, a reclassification before the receipt or into a locked month refused, the deposits invariant; D — the exception list's deadline keys on the receipt; 4 — the write-off with relief (every condition refused by name), box 7 in the claim period, no longer outstanding anywhere, the recovery document end to end (draft → issue: E-40(9) exactly, the receipt applied, +2,000/+300 in the payment's month, the XML's 388/BillingReference/KSA-5/IssueDate, a second recovery bounded, the statement and audit chains); a MIGRATED relief (allocation first, VAT leg only, rate never defaulted, not in our box 7); every journal balanced, no recovery ever on AR/SALES/bank, no write-off ever on a deposit liability. AP-1 (16), AP-2 (16), AP-3 (10) re-pointed to the new rules where the rules changed (a classification now moves between liabilities; the 386 has one date; a locked month fails closed) and green; AP-4 (27) unchanged.
- `apps/web/e2e/ap-bad-debt.spec.ts` (4, by clicking): English desktop — *Write off* → the dialog (unpaid, VAT relieved, the certificate required before submit) → "Written off" badge, Due 0.00 → a receipt on account (UNIDENTIFIED_RECEIPTS) → *Declare recovery* → the receipt picker → the draft's "Tax invoice — recovery (Art. 40(9))" badge, Due "—" → approved: `paid`, the receipt at zero, box 6 +150; English phone (both dialogs fit; no sideways scroll); Arabic RTL desktop («مشطوبة», «استرداد»); Arabic RTL phone. One seed defect found by the walk and fixed in the spec: a receipt dated by the browser's UTC day preceded a relief claimed on the Riyadh business day — the night-window seam, in the harness, not the product.

## 17.6 What remains

| Item | Who | Status |
| --- | --- | --- |
| **Answers 3 and 5** (Fatoora-only credit notes on historical documents; migration corrections to opening retained earnings) and the migrated relief's writer | engineering — the migration follow-ups (§18) | next |
| **Z1** | ZATCA | unchanged |
| **Simulation / production** for the changed shapes | the owner (the entity) | as for every document type |
| **BT-115 on the recovery invoice** (= total, the amount having been received) | advisor | reasoned-not-verified; the sandbox accepts it |
