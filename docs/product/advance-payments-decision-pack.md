...
**Status (2026-09-20): §1–§12 are the research and decision record (not re-argued). AP-1 — deposit classification + the VAT-review list — is BUILT on `feat/ap-1-deposit-classification` (§13, the as-built record); AP-2/AP-3/AP-4 are NOT built and AP-2 is BLOCKED on the accountant's A1/A2 and the XML Implementation Standard ¶9.5.** The Batch 1B payment core that this pack builds on is IMPLEMENTED and merged (`main` `a290d079`; record: known-issues file, "BATCH 1B — CLOSED 2026-09-17"). The live exposure AP-1 makes visible is recorded where a future session will look: known-issues file, "ADVANCE VAT UNDER-DECLARATION". Current state authority: [CLAUDE.md §2](../../CLAUDE.md).
