...
**Status (2026-09-21): §1–§12 are the research and decision record (not re-argued). AP-1 (§13) and AP-2 (§14 — the advance tax invoice type 386, its E2 entry, the final invoice's prepayment adjustment with the E3 entry, the ¶9.5 UBL, the return reading both) are BUILT on `feat/ap-2-advance-tax-invoice` (on top of `feat/ap-1-deposit-classification`), NOT merged. 🔴 A1, A2 and A3 are RESOLVED by the accountant (2026-09-21; §8 carries the answers verbatim); Z1 stays OPEN with ZATCA. AP-3's remaining scope is the credit note against a 386 and the refund precondition it unlocks (§14.6); AP-4 (the live sandbox pass for a 386 and a 388-with-adjustment) is the gate that stands between this build and a real taxpayer.** The Batch 1B payment core that this pack builds on is IMPLEMENTED and merged (`main` `a290d079`; record: known-issues file, "BATCH 1B — CLOSED 2026-09-17"). The live exposure AP-1 made visible and AP-2 closes at the document is recorded where a future session will look: known-issues file, "ADVANCE VAT UNDER-DECLARATION". Current state authority: [CLAUDE.md §2](../../CLAUDE.md).
...
| **AP-3 — the credit note against a 386 (E5) and the refund it unlocks** (the adjustment half of the original AP-3 shipped inside AP-2) | Credit note (381) referencing a 386: `Dr VAT Payable / Cr Customer deposits`, cleared/reported; then the D-4 refund of the released remainder. Until built, both are REFUSED by name (`note_original_is_advance_invoice`; `advance_invoiced_requires_prepayment_adjustment`) — fail-closed, never a wrong entry. The migrated `invoiced` arm stays behind **Z1**. | AP-2 ✅; **Z1** for the migrated arm only | ~1 week |
...
| **AP-3 — the credit note against a 386, and the refund it unlocks** | engineering | refunding or cancelling an invoiced advance | Refused today by name at both doors (`note_original_is_advance_invoice`; `advance_invoiced_requires_prepayment_adjustment`). E5's entry shape (`Dr VAT Payable / Cr Customer deposits`) is in §6; the Art. 40(5) period rule applies. |
