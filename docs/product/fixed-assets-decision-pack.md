...
**Status (2026-09-21): RESEARCH AND DESIGN ONLY — nothing built, no migration, no schema, no accounting or VAT logic changed. One research pass, run on `feat/fixed-assets-research` on top of the Advance Payments stack (PR #165). Two genuine accountant decisions are outstanding (§15); implementation is blocked on them AND on the Advance Payments VAT-period answer (§15, "Advance Payments — accountant decision pending"). Current state authority: [CLAUDE.md §2](../../CLAUDE.md).**
...
- **Accountant decisions outstanding:** **FA-1** (income-tax basis in scope?), **FA-2** (VAT capital-asset regime — v1 computation scope and the two readings). Nothing else.
- **Implementation blocked by those decisions:** YES — FA-0 gates FA-A.

**ADVANCE PAYMENTS — ACCOUNTANT DECISION PENDING.** The VAT-period / 386-date question (advance-payments pack §16.5) is unanswered. Fixed Assets RESEARCH proceeded because it does not depend on it; Fixed Assets IMPLEMENTATION must not begin until that answer is in and, if it changes AP-2's posting date or return reading, until that AP correction is implemented and merged first.
