...
| **B3** | 🔴 **STAGED CAPTURES CANNOT BE DELETED ON CLOUD — and the purge job destroys its own record of what it failed to delete.** `ArchiveStore` has no `delete` by design and must stay that way, so `stagingStore.remove` reaches past the interface for `local-fs` and is a **silent no-op on `supabase-storage`** (`stagingStore.ts:69-80`). `purgeOnce` then deletes the metadata row regardless (`promotion.service.ts:96-99`, `remove(...).catch(() => {})`), so on cloud the bytes are orphaned **and the index to them is gone** — you cannot enumerate, retry, or even prove what was left behind. **This now applies to a user-reachable act:** `POST /capture/:id/discard` is in the product, so a user who photographs the wrong thing (or their ID card in frame) is told it is discarded and on cloud it is not. Fix with a **separate deletable-staging interface** — never by weakening `ArchiveStore` — and make purge **delete the bytes BEFORE the row**, failing the row's deletion if the bytes survive.<br><br>**Reframed 2026-08-14 (audit):** written when the capture pipeline had no production caller — it was reasoning about a pipeline nothing fed. Real bytes stage now, so this is live, and the deletable window is **bounded by posting**: promotion copies the photograph into `ArchiveStore`, which by design can never delete it again. | The **PDPL problem in concrete form**, no longer hypothetical: staging exists precisely so an abandoned photograph — possibly a third party's personal data — does not become permanent, and on cloud the mechanism does not deliver the one thing it was built for. **B3, C7 and C8 are one question in three parts** — B3 is the technical half of whatever C8 answers, and it is now the half that already has users. |
