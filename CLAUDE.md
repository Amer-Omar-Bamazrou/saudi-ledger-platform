...
| **C12** | 🔴 **HIGH — NEAR THE FRONT OF THE NEXT NON-FEATURE PASS (owner, 2026-08-20). `invoices.invoice_number` HAS NO UNIQUE CONSTRAINT, AND IT IS THE ZATCA `cbc:ID`.**<br><br>**The three facts together are why this is severity HIGH rather than housekeeping:** (1) the number is written directly as **`cbc:ID`** (`ubl/…:183`) — the invoice's legal identifier in the transmitted ZATCA document — and as the **`BillingReference`** a credit/debit note uses to name what it corrects (`:201`); (2) **ZATCA requires unique, sequential invoice numbering**, so a duplicate is a compliance failure and not merely a UX wart; (3) **no constraint exists anywhere** — the only unique index on the table is `invoices_company_icv_unq`, so duplicates are accepted silently.<br><br>**Two live sources of duplicates.** The manual form mints the number **in the browser** as `` INV-${Date.now().toString().slice(-6)} `` (`Invoices.tsx:36`) — a truncated millisecond clock, and any client may POST any string. And M21.2's converted invoices use a **server-side** `INV-{YYYY}-{NNNN}` allocator which is better but, unlike the quotation allocator, **has no backstop**: `quotations_company_number_unq` turns a lost race into a failed insert, and there is no equivalent here, so a lost race duplicates. M21.2 reduced the blast radius; it explicitly did NOT close this.<br><br>**Consequence if it fires:** two ZATCA documents sharing an identifier, and a credit note whose `BillingReference` is ambiguous about which invoice it corrects — while `original_invoice_id` (the real FK) stays correct, so **the database and the transmitted document would disagree**.<br><br>🔴 **Verified:** the code path, the missing constraint, both allocators. **NOT verified and must not be restated as fact until it is:** the exact ZATCA/EN-16931 wording on uniqueness *and* sequentiality — that needs C9's primary-source treatment (read the e-invoicing resolution, cite the article) as the FIRST step of the fix, because the wording decides whether gaps are permitted and therefore what the allocator may do.<br><br>**Fix shape (not started):** server-side allocation as the single path for ALL invoices + `UNIQUE (company_id, invoice_number)`, plus a decision on duplicates already present in tenant data. | Design of M21 §3; `Invoices.tsx:36`, `invoices.ts:113`, `ubl/…:183,201`, `quotations.repository.ts` (`nextInvoiceNumber`) |
| C11 | **Read the GCC VAT Framework Agreement for `GOVT_FEES`, `GOVT_GRANTS`, `GOSI_*`** (owner-directed, 2026-08-19). Their O defaults rest on sovereign-capacity and no-supply rules that live in the Agreement (Art. 9 area), which C9's pass deliberately did NOT resolve by reasoning. Likely desk research in the same shape as C9 (primary text, per-article citations); advisor only if the text is ambiguous. | C9 doc §assumed |
