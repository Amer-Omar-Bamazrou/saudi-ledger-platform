...
| C6 | **Data residency / hosting region — now ALSO the AI hosting decision (2026-08-18).** ZATCA permits cloud (the "must be in KSA" claim was a secondary-source error); NCA / sector rules are **unverified legal questions**. Choose host, KMS region, **and GPU hosting for the self-hosted models together** — the owner's data-boundary answer rules out hosted model APIs, so the AI layer needs in-Kingdom (or in-platform) GPUs: verify STC / Ezditek / DataVolt / HUMAIN-era / AWS-KSA availability against providers, never from memory. Self-hosting floor stated in design-ai-layer §12 (~$3–4.5k/month, one replica, no redundancy, bursty-idle shape). No hosted Supabase project exists yet — this is a deployment decision, not a migration. | Residency correction, phase-2 history + [`design-ai-layer.md`](docs/product/design-ai-layer.md) |
