...
| **2** | 🔴 **11 of the 14 expense-account picks POST TO PURCHASES under the chosen label** — `lib/accounts.ts` names are matched by text in `bills.approvable.ts`; only 3 exist in the seeded chart, the rest fall back silently and the line displays the name the user chose. The one item in the English-content count that is wrong in the books today (findings file, "THE ENGLISH-CONTENT DEPENDENCY COUNT": 1 needs-an-id · 2 + 364 needs-a-code · 51 + 15 display-only). | Posts + hides — the triage check's pair. | Fix shape recorded; NOT fixed (count-only order). Owner to rank. |
| **2b** | **`operatorService.getApplication` accepts ANY orgId**, including an approved LIVE tenant, returning CR/VAT and verification documents; the access **never expires**. | **C8 (PDPL)** — a legal question, not a code one. | Audited and operator-only, so not a hole; an unbounded retention surface. Ask the advisor before building an expiry. |
