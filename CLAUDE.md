...
arrived on a schedule). 🔴 OPEN: **hosting** — now a four-option decision
recorded with real numbers in the spec's §12a annex (Groq-Dammam+ZDR at
~$0.50–1/tenant/mo vs local-images hybrid ~$1–1.5k/mo vs full self-host
~$3–4.5k/mo floor; the three readings of the Q5 data boundary decide it,
and that choice is the owner's) — plus the Arabic benchmark, the exact
vision-model ID (path-dependent: Qwen vs Llama 4 vision), and the eval
gate's thresholds. Two C6 verify items owner-directed: Groq Dammam
Enterprise pinning terms; Arabic-acceptable vision model in that region.
See [`docs/product/design-ai-layer.md`](docs/product/design-ai-layer.md).
...
| C6 | **Data residency / hosting region — now ALSO the AI hosting decision (2026-08-18).** ZATCA permits cloud (the "must be in KSA" claim was a secondary-source error); NCA / sector rules are **unverified legal questions**. Choose host, KMS region, **and AI hosting together** — a four-option decision with real numbers in design-ai-layer §12a (Groq-Dammam Enterprise + ZDR / local-images hybrid / full self-host / the deterministic floor), turning on WHICH READING of the owner's data-boundary answer was meant — that choice is the owner's. 🔴 Verify inside C6, owner-directed: (1) Groq Enterprise terms + pricing for Dammam region pinning; (2) an Arabic-acceptable vision model available in that region (Groq path likely = Llama 4 vision, not Qwen; the §2a Arabic gate decides on measured numbers). If self-hosting: in-Kingdom GPU availability (STC / Ezditek / DataVolt / HUMAIN-era / AWS-KSA) verified against providers, never from memory; floor ~$3–4.5k/month, one replica, no redundancy, bursty-idle shape. No hosted Supabase project exists yet — this is a deployment decision, not a migration. | Residency correction, phase-2 history + [`design-ai-layer.md`](docs/product/design-ai-layer.md) |
