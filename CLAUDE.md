...
**2026-09-22** — 🔴 **FIXED ASSETS FA-A…FA-D BUILT** ([`pack`](docs/product/fixed-assets-decision-pack.md) §20–§23): accounts, categories (Art. 17 group · Art. 52 class · account triple), a register whose figures are DERIVED from the schedule (posted rows FROZEN); a BILL capitalises what it buys (non-deductible VAT into cost); the run posts Dr expense / Cr accumulated (a closed month fails closed); estimate changes prospective. 🔴 **FA-C** (§22): a SALE is an invoice naming the asset (the gain is OTHER income; `SALES` never moves), a scrap its own act, both depreciated to the disposal month first; Art. 50(3)/52(7)/(8) per kind. 🔴 **FA-D** (§23): migrated assets are a 1C staging set posting NO line — the register RECONCILES to the trial balance, then becomes rows on the opening journal resuming their schedules.
...
**Fixed assets: IN BUILD** — [`pack`](docs/product/fixed-assets-decision-pack.md) (FA-1/FA-2 answered; FA-A…FA-D built; FA-E/FA-F follow).
