...
| **L1** | 🔴 **THE INVOICE CANNOT LEAVE THE PRODUCT** — no PDF, no print view, no share artifact (launch blocker, 2026-09-01, found by WALKING the core path). **A simplified invoice's QR exists to be PRESENTED to the customer, and no artifact can present it.** 🔴 **Design AGREED 2026-09-02, not built:** [`design-invoice-document.md`](docs/product/design-invoice-document.md) — the single writer for it. Settled there from the **primary Arabic** regulation: **Arabic is mandatory, other languages are permitted alongside** (so wholly-English is non-compliant and bilingual is explicitly contemplated); ZATCA constrains the FIELD SET, **not the layout**, so customisation is levels 1–2 with a fixed mandatory core, and level 3 is *unnecessary, not forbidden*. Target **PDF/A-3 with embedded XML** from the start. | Build to that design; "send" follows once B1 is wired. Every document type reuses the artifact. |
