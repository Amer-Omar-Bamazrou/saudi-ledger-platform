...
| **C12** | 🔴 **HIGH — INVOICE NUMBERS ARE CLIENT-GENERATED AND NOT UNIQUE.** Found 2026-08-20 while designing M21's numbering (deliberately not copied there). Two facts, both verified: (1) the number is minted **in the browser** as `` INV-${Date.now().toString().slice(-6)} `` (`apps/web/src/pages/Invoices.tsx:36`) — a 6-digit truncation of a millisecond clock, and any client can POST any string; (2) **no uniqueness constraint exists** on `invoices.invoice_number` in any migration or in the Drizzle schema — the only unique index on the table is `invoices_company_icv_unq`. So duplicates are accepted silently.<br><br>**Why this is compliance, not untidiness:** `invoiceNumber` is written directly as **`cbc:ID`** (`ubl/…:183`), the invoice's legal identifier in the ZATCA document, and as the **`BillingReference`** a credit/debit note uses to name the invoice it corrects (`:201`). A collision therefore means two ZATCA documents sharing an identifier, and a note whose reference is ambiguous about which invoice it corrects — while `originalInvoiceId` (the real FK) stays correct, so the DB and the transmitted document would disagree. 🔴 **Verified:** the code path and the missing constraint. **ASSUMED, needs the C9 treatment against the primary source before the fix is designed:** the exact ZATCA/EN-16931 wording on uniqueness *and sequentiality* (BR-02 requires an identifier; the KSA e-invoicing resolution's sequential-numbering requirement has NOT been read here — do not restate it as fact until it has been). **Fix shape (not started):** server-side allocation + `UNIQUE (company_id, invoice_number)`, plus a decision on what to do about any duplicates already in a tenant's data. | Design of M21 §3; `Invoices.tsx:36`, `invoices.ts:113`, `ubl/…:183,201` |
