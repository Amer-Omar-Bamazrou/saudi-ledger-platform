...
| --- | --- | --- | --- |
| **1** | **Password recovery — 🔴 DECIDED (owner, 2026-09-04): the record's OPTION C, by mechanism.** Operator break-glass reset ships NOW (audited, on the operator surface — the F1-shaped standing capability the 2026-08-30 record warns about, accepted knowingly); self-service email reset follows WHEN THE MAIL PROVIDER LANDS, and the owner picks the provider. 🔴 The gap, recorded not glossed: until the email flow exists, a locked-out solo admin — the common SME case, not an edge — has no self-service route; their only path is contacting us for an operator reset. 🔴 The known risk of C is that the break-glass ships and the email flow never does; the mail provider unblocking B1 + invoice-send + this is what prevents it. (⚠ The owner's message said "A now, B when email exists" — letters transposed vs the 2026-08-30 record, where A=email, B=operator; recorded by MECHANISM, which was unambiguous.) | Build the break-glass now; the email flow is `organization_invitations`-shaped and waits only on the provider. |
